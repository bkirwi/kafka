...
public class IdempotentProducerPerformance {

    public static void main(String[] args) throws Exception {
        if (args.length < 4) {
            System.err.println("USAGE: java " + IdempotentProducerPerformance.class.getName() +
        ...
        // This demo uses the consumer interface as a very roundabout way to get the partition's upcoming offset
        ...
        while (true) {
            ...
            // If we've had an error, reset the offset to the upcoming offset for the partition
            if (stats.hasError) {
                producer.flush();
                stats.hasError = false;
                consumer.seekToEnd(topicPartition);
                offset = consumer.position(topicPartition);
                System.err.println("Recovered to offset " + offset);
            }

            // If we've successfully sent `numRecords` messages, we're done!
            if (offset == numRecords) {
                producer.flush();
                if (stats.hasError) continue;
                else break;
            }

            // Create and send the record, specifying the expected offset
            byte[] payload = String.format(format, offset).getBytes();
            ProducerRecord<byte[], byte[]> record = new ProducerRecord<byte[], byte[]>(topicName, 0, offset, null, payload);
            long sendStartMs = System.currentTimeMillis();
            Callback cb = stats.nextCompletion(sendStartMs, payload.length, stats);
            producer.send(record, cb);

            if (throttler.shouldThrottle(offset, sendStartMs)) {
                throttler.throttle();
            }
    ...
    private static final class PerfCallback implements Callback {
        ...
        public void onCompletion(RecordMetadata metadata, Exception exception) {
            ...
            if (exception != null) {
                if (exception instanceof ExpectedOffsetMismatchException) {
                    if (!stats.hasError) {
